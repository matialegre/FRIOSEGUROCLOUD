...
class MainActivity : AppCompatActivity() {
    ...
    private val dataReceiver = object : BroadcastReceiver() {
        override fun onReceive(context: Context?, intent: Intent?) {
            intent?.let {
                ...
                val tempOverCritical = it.getBooleanExtra("temp_over_critical", false)
                val highTempElapsedSec = it.getIntExtra("high_temp_elapsed_sec", 0)
                val alertDelaySec = it.getIntExtra("alert_delay_sec", 300)
                
                // Configuración del dispositivo (se actualiza automáticamente)
                val tempCritical = it.getDoubleExtra("temp_critical", -10.0)
                val defrostCooldownSec = it.getIntExtra("defrost_cooldown_sec", 1800)
                val defrostNC = it.getBooleanExtra("defrost_relay_nc", false)
                
                // Actualizar UI de configuración si hay datos válidos
                if (tempCritical != -10.0 || alertDelaySec != 300) {
                ...
                updateDisplay(temp, temp1, temp2, doorOpen, relayOn, alertActive, alertAcknowledged, alertMsg, uptime, rssi, internet, location, deviceIp, defrostMode, cooldownMode, cooldownRemainingSec, tempOverCritical, highTempElapsedSec, alertDelaySec)
    ...
    // Device ID para buscar en Supabase (REEFER_DEV_BHI para Desarrollo)
    private val TARGET_DEVICE_ID = "REEFER_DEV_BHI"
    
    @Volatile private var discoveredIp: String? = null
    
    private fun discoverEsp32() {
        Toast.makeText(this, "🔍 Buscando Reefer DEV en la red...", Toast.LENGTH_SHORT).show()
        discoveredIp = null
        
        val threads = mutableListOf<Thread>()
        
        // Método 1: mDNS
        threads.add(kotlin.concurrent.thread {
            tryMdns()
        })
        
        // Método 2: Supabase IP
        threads.add(kotlin.concurrent.thread {
            trySupabaseIp()
        })
        
        // Método 3: Escanear IPs conocidas
        threads.add(kotlin.concurrent.thread {
            tryScanKnownIps()
        })
        ...
        kotlin.concurrent.thread {
            ...
            runOnUiThread {
                if (finalIp != null) {
                    ...
                    Toast.makeText(this, "✅ Reefer DEV encontrado: $finalIp", Toast.LENGTH_SHORT).show()
                } else {
                    etServerIp.setText("")
                    etServerIp.hint = "IP no encontrada - ingresa manualmente"
                    Toast.makeText(this, "❌ No se encontró Reefer DEV", Toast.LENGTH_LONG).show()
                }
            }
        }
    }
    ...
    private fun tryMdns() {
        ...
        try {
            val url = java.net.URL("http://reefer.local/api/status")
    ...
    private fun trySupabaseIp() {
        ...
        try {
            ...
            if (conn.responseCode == 200) {
                ...
                if (!ip.isNullOrEmpty() && ip != "null" && testReeferIp(ip)) {
                    discoveredIp = ip
                }
            }
            conn.disconnect()
    ...
    private fun tryScanKnownIps() {
        ...
        for (future in futures) {
            ...
            try {
                ...
                if (result != null && discoveredIp == null) discoveredIp = result
            } catch (e: Exception) { }
        }
        
        executor.shutdownNow()
        if (discoveredIp == null) scanCurrentSubnet()
    }
    
    private fun testReeferIp(ip: String): Boolean {
        return try {
            val url = java.net.URL("http://$ip/api/status")
            val conn = url.openConnection() as java.net.HttpURLConnection
    ...
    private fun scanCurrentSubnet() {
        ...
        try {
            ...
            val baseIp = String.format("%d.%d.%d", ipInt and 0xff, ipInt shr 8 and 0xff, ipInt shr 16 and 0xff)
            val executor = java.util.concurrent.Executors.newFixedThreadPool(20)
            val ipsToScan = (100..120).map { "$baseIp.$it" }
    ...
    private fun updateDisplay(
        ...
        tempOverCritical: Boolean = false, highTempElapsedSec: Int = 0, alertDelaySec: Int = 300
    ) {
        ...
        // Alert banner - mostrar si hay alerta (activa o silenciada) o esperando
        if (alertActive) {
            alertBanner.visibility = View.VISIBLE
            alertBanner.setBackgroundColor(Color.parseColor("#F44336"))  // Rojo
            if (alertAcknowledged) {
                tvAlertMessage.text = "🔕 SILENCIADA - $alertMsg"
            } else {
                tvAlertMessage.text = alertMsg
            }
        } else if (tempOverCritical && highTempElapsedSec > 0 && highTempElapsedSec < alertDelaySec) {
            // NUEVO: Temperatura sobre crítico, esperando tiempo de espera
            val remaining = alertDelaySec - highTempElapsedSec
            val minR = remaining / 60
            val secR = remaining % 60
            
            alertBanner.visibility = View.VISIBLE
            alertBanner.setBackgroundColor(Color.parseColor("#FF9800"))  // Naranja
            tvAlertMessage.text = "⚠️ TEMP ALTA - ALERTA EN ${minR}:${String.format("%02d", secR)}"
        } else {
    ...
    private fun saveConfig(view: View, ip: String) {
        ...
        android.util.Log.d("MainActivity", "📤 Enviando config a $ip: $json")
        ...
        thread {
            try {
                ...
                android.util.Log.d("MainActivity", "📥 Respuesta ESP32: $responseCode - $response")

                runOnUiThread {
                    if (responseCode == 200) {
                        Toast.makeText(this, "✅ Configuración guardada en ESP32", Toast.LENGTH_SHORT).show()
                        // Recargar valores configurados en la pantalla principal
                        loadConfigValues(ip)
                    } else {
                        Toast.makeText(this, "⚠️ ESP32 respondió: $responseCode", Toast.LENGTH_LONG).show()
                    }
                }
            } catch (e: Exception) {
                android.util.Log.e("MainActivity", "❌ Error enviando config: ${e.message}")
                runOnUiThread {
