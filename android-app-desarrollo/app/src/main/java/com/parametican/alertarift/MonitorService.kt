...
class MonitorService : Service() {
    ...
    private fun processResponse(json: String) {
        try {
            ...
            // Obtener configuración del dispositivo (viene en "thresholds" del ESP32)
            val thresholds = data.optJSONObject("thresholds")
            val tempCritical = thresholds?.optDouble("temp_critical", -10.0) ?: -10.0
            val alertDelaySec = thresholds?.optInt("alert_delay_sec", 300) ?: system.optInt("alert_delay_sec", 300)
            val defrostCooldownSec = thresholds?.optInt("defrost_cooldown_sec", 1800) ?: 1800
            val features = data.optJSONObject("features")
            val defrostNC = features?.optBoolean("defrost_relay_nc", false) ?: false
            
            // Enviar broadcast a la UI
            val updateIntent = Intent("REEFER_DATA_UPDATE").apply {
                ...
                putExtra("temp_over_critical", system.optBoolean("temp_over_critical", false))
                putExtra("high_temp_elapsed_sec", system.optInt("high_temp_elapsed_sec", 0))
                putExtra("alert_delay_sec", alertDelaySec)
                // Configuración para actualizar UI
                putExtra("temp_critical", tempCritical)
                putExtra("defrost_cooldown_sec", defrostCooldownSec)
                putExtra("defrost_relay_nc", defrostNC)
            }
