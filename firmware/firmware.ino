...
// ============================================
// SUPABASE CON COLA OFFLINE
// ============================================
unsigned long lastSupabaseSync = 0;

void addToOfflineQueue() {
  offlineQueue[offlineQueueIndex].temperature = sensorData.tempAvg;
  offlineQueue[offlineQueueIndex].doorOpen = sensorData.doorOpen;
  offlineQueue[offlineQueueIndex].alertActive = state.alertActive;
  offlineQueue[offlineQueueIndex].timestamp = millis();
  offlineQueue[offlineQueueIndex].pending = true;
  
  offlineQueueIndex = (offlineQueueIndex + 1) % OFFLINE_QUEUE_SIZE;
  Serial.println("[OFFLINE] Dato guardado en cola");
}

void syncOfflineQueue() {
  if (!state.internetAvailable) return;
  if (strlen(SUPABASE_URL) < 10 || strcmp(SUPABASE_ANON_KEY, "TU_ANON_KEY_AQUI") == 0) return;
  ...
  for (int i = 0; i < OFFLINE_QUEUE_SIZE; i++) {
    if (offlineQueue[i].pending) {
      ...
      doc["temperature"] = offlineQueue[i].temperature;
      doc["door_open"] = offlineQueue[i].doorOpen;
      doc["alert_level"] = offlineQueue[i].alertActive ? "warning" : "normal";
      ...
      if (code == 201) {
        ...
        Serial.printf("[SUPABASE] Sincronizado dato offline #%d\n", i);
      }
      http.end();
...
void syncToSupabase() {
  if (millis() - lastSupabaseSync < 300000) return;
  if (strlen(SUPABASE_URL) < 10 || strcmp(SUPABASE_ANON_KEY, "TU_ANON_KEY_AQUI") == 0) return;
  ...
  // Primero sincronizar cola offline
  syncOfflineQueue();
  
  // Luego enviar dato actual
  HTTPClient http;
  String url = String(SUPABASE_URL) + "/rest/v1/temperature_readings";
  ...
  StaticJsonDocument<256> doc;
  doc["device_id"] = DEVICE_ID;
  doc["temperature"] = sensorData.tempAvg;
  doc["door_open"] = sensorData.doorOpen;
  doc["relay_active"] = state.relayState;
  doc["alert_level"] = state.criticalAlert ? "critical" : (state.alertActive ? "warning" : "normal");
  doc["rssi"] = WiFi.RSSI();
  ...
  int code = http.POST(body);
  if (code == 201) {
    Serial.println("[SUPABASE] Sincronizado");
  }
  http.end();
...
void sendAlertToSupabase(String alertType, String message) {
  if (!state.internetAvailable) return;
  if (strlen(SUPABASE_URL) < 10 || strcmp(SUPABASE_ANON_KEY, "TU_ANON_KEY_AQUI") == 0) return;
